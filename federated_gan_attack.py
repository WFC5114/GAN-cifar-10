...
test_images = (test_images) / 255   # Normalization
...
# Sample to warm up
warm_up_data = train_images[0:500000]
warm_up_labels = train_labels[0:500000]
...
def make_discriminator_model():
    ...
    model.add(keras.layers.Conv2D(64, (3, 3), strides=(2, 2), padding='same', input_shape=[32, 32, 3]))
...
for i in range(Clinets_per_round):
    ...
    Models[i].compile(optimizer=keras.optimizers.Adam(learning_rate=1e-5),
...
discriminator_optimizer = tf.keras.optimizers.SGD(learning_rate=1e-3, decay=1e-7)
...
model.fit(warm_up_data, warm_up_labels, validation_split=0, epochs=300, batch_size = 256)
...
for r in range(Round):
    ...
    for i in range(Clinets_per_round):
        ...
        if r != 0 and i == 0 and Test_accuracy[i-1] > 0.6:
